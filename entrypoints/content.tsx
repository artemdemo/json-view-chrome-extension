import {
  useEffect,
  useMemo,
  useRef,
  useState,
  type MouseEventHandler,
} from 'react';
import { createRoot } from 'react-dom/client';
import '../styles/content.css';

const JSON_MIME_TYPE = 'application/json';
const COPY_RESET_DELAY_MS = 1_500;

export default defineContentScript({
  matches: ['http://*/*', 'https://*/*'],
  runAt: 'document_idle',
  main() {
    prettifyJsonDocument();
  },
});

function prettifyJsonDocument() {
  if (!isJsonDocument(document)) {
    return;
  }

  const source = getDocumentText(document);
  if (source.trim() === '') {
    return;
  }

  try {
    const json = JSON.parse(source) as unknown;
    const prettyJson = JSON.stringify(json, null, 2);

    renderJsonViewer(prettyJson, json, source);
  } catch {
    // The server advertised JSON, but the body is not parseable. Leave it alone.
  }
}

function isJsonDocument(doc: Document): boolean {
  return normalizeMimeType(doc.contentType) === JSON_MIME_TYPE;
}

function normalizeMimeType(contentType: string): string {
  return contentType.split(';', 1)[0].trim().toLowerCase();
}

function getDocumentText(doc: Document): string {
  const { body } = doc;
  if (!body) {
    return '';
  }

  const onlyChild = body.children.length === 1 ? body.firstElementChild : null;
  if (onlyChild?.tagName === 'PRE') {
    return onlyChild.textContent ?? '';
  }

  return body.textContent ?? '';
}

function renderJsonViewer(prettyJson: string, json: unknown, source: string) {
  document.documentElement.classList.add('json-view-extension-document');

  const root = document.createElement('div');
  root.id = 'json-viewer-react-root';

  document.body.replaceChildren(root);
  createRoot(root).render(
    <JsonViewer json={json} prettyJson={prettyJson} source={source} />,
  );
}

type JsonViewerProps = {
  json: unknown;
  prettyJson: string;
  source: string;
};

type CopyState = 'idle' | 'copied' | 'failed';

function JsonViewer({ json, prettyJson, source }: JsonViewerProps) {
  const [copyState, setCopyState] = useState<CopyState>('idle');
  const copyResetTimeout = useRef<number | undefined>(undefined);
  const highlightedJson = useMemo(() => highlightJson(prettyJson), [prettyJson]);

  useEffect(() => {
    return () => {
      if (copyResetTimeout.current !== undefined) {
        window.clearTimeout(copyResetTimeout.current);
      }
    };
  }, []);

  const showCopyState = (nextCopyState: CopyState) => {
    setCopyState(nextCopyState);

    if (copyResetTimeout.current !== undefined) {
      window.clearTimeout(copyResetTimeout.current);
    }

    copyResetTimeout.current = window.setTimeout(() => {
      setCopyState('idle');
    }, COPY_RESET_DELAY_MS);
  };

  const handleCopy: MouseEventHandler<HTMLButtonElement> = async () => {
    try {
      await navigator.clipboard.writeText(prettyJson);
      showCopyState('copied');
    } catch {
      showCopyState(fallbackCopy(prettyJson) ? 'copied' : 'failed');
    }
  };

  const copyLabel =
    copyState === 'copied' ? 'Copied' : copyState === 'failed' ? 'Failed' : 'Copy';

  return (
    <main className="json-viewer-root" data-json-viewer="true">
      <header className="json-viewer-toolbar">
        <div className="json-viewer-summary">
          <h1>JSON</h1>
          <p>
            {getJsonKind(json)} - {formatBytes(getByteLength(source))}
          </p>
        </div>

        <button className="json-viewer-copy" type="button" onClick={handleCopy}>
          {copyLabel}
        </button>
      </header>

      <pre className="json-viewer-pre">
        <code dangerouslySetInnerHTML={{ __html: highlightedJson }} />
      </pre>
    </main>
  );
}

function getJsonKind(value: unknown): string {
  if (Array.isArray(value)) {
    return `array (${value.length})`;
  }

  if (value !== null && typeof value === 'object') {
    return `object (${Object.keys(value).length})`;
  }

  return value === null ? 'null' : typeof value;
}

function getByteLength(value: string): number {
  return new Blob([value]).size;
}

function formatBytes(bytes: number): string {
  if (bytes < 1024) {
    return `${bytes} B`;
  }

  const units = ['KB', 'MB', 'GB'];
  let size = bytes / 1024;
  let unitIndex = 0;

  while (size >= 1024 && unitIndex < units.length - 1) {
    size /= 1024;
    unitIndex += 1;
  }

  return `${size.toFixed(size >= 10 ? 1 : 2)} ${units[unitIndex]}`;
}

function highlightJson(json: string): string {
  const tokenPattern =
    /("(?:\\u[\da-fA-F]{4}|\\[^u]|[^\\"])*"(\s*:)?|\btrue\b|\bfalse\b|\bnull\b|-?\d+(?:\.\d*)?(?:[eE][+-]?\d+)?)/g;

  return escapeHtml(json).replace(tokenPattern, (token) => {
    let tokenType = 'number';

    if (token.startsWith('"')) {
      tokenType = token.endsWith(':') ? 'key' : 'string';
    } else if (token === 'true' || token === 'false') {
      tokenType = 'boolean';
    } else if (token === 'null') {
      tokenType = 'null';
    }

    return `<span class="json-token json-token-${tokenType}">${token}</span>`;
  });
}

function escapeHtml(value: string): string {
  return value
    .replaceAll('&', '&amp;')
    .replaceAll('<', '&lt;')
    .replaceAll('>', '&gt;');
}

function fallbackCopy(value: string): boolean {
  const textarea = document.createElement('textarea');
  textarea.value = value;
  textarea.setAttribute('readonly', 'true');
  textarea.className = 'json-viewer-copy-buffer';

  document.body.append(textarea);
  textarea.select();

  try {
    return document.execCommand('copy');
  } finally {
    textarea.remove();
  }
}
